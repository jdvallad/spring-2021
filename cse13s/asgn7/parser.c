...
//
// Returns the lowercase version of char c. If c does not have a lowercase
// equivalent, then just c is returned.
//
...
//
// Converts a string c into lowercase. Will modify the original string, so use
// this with caution.
//
...
char *next_word(FILE *infile, regex_t *word_regex) {
    ...
    index = (index + 1) % count;
    convert_to_lower(word); // converts word to lowercase
    return word;
}
